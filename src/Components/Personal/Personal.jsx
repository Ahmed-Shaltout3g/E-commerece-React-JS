...
export default function Personal() {
    let { personal } = useContext(mediaContext)
    ...
    return <>
        ...
        {personal?.length ? <div className="container mt-5">

            <div className="row g-4">

                {!personal ? <div className='h-100 d-flex justify-content-center align-items-center'><div class="spinner-grow  " role="status">
                    <span className="visually-hidden">Loading...</span>
                </div></div> :
                    personal?.map((item, index) => (
                        <div key={index} className="col-md-3 grid-box   ">
                            <div className='bg-white  border-1 border'>
                                <Link className=' text-dark  ' to={`/details/${item.id}/personalcare`}>
                                    <div className=' '>
